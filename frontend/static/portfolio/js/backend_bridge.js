(function () {
  // ── API Base URL ─────────────────────────────────────────────────────────
  // Reads from window.__elaApiBase (set by config.js before this script).
  // Falls back to empty string (same-origin) for backward compatibility when
  // Django still serves index.html directly (e.g. local monorepo dev).
  const API_BASE = (window.__elaApiBase || "").replace(/\/+$/, "");

  /**
   * Convert a relative path like "/api/bootstrap/" into an absolute URL
   * pointing at the backend. Absolute URLs (http/https) are returned as-is.
   */
  function apiUrl(path) {
    if (!path) return path;
    if (/^https?:\/\//i.test(path)) return path;  // already absolute
    if (!API_BASE) return path;                    // same-origin fallback
  ...
  function openDjangoAdmin() {
    ...
    window.location.href = apiUrl(window.djangoAdminUrl || "/django-admin/");
  }
  ...
  async function beginSecureVideoDownload(video, ownerUsername = "") {
    ...
    if (video.download_url && videoDownloadAccessState(video, ownerUsername) === "owner") {
      window.location.assign(apiUrl(video.download_url));
      return;
    }
    ...
    window.location.assign(apiUrl(payload.download_url));
  }
  ...
  // When Django serves index.html directly it injects the bootstrap JSON via
  // <script id="ela-bootstrap">…</script> and we use it immediately.
  // When the frontend is deployed as a standalone static site, that element
  // is absent. In that case we fetch /api/bootstrap/ from the backend.
  if (bootstrapElement) {
    // Fast path: Django-injected JSON is already available.
    replaceState(bootstrap);
  } else {
    // Standalone path: apply empty state first so the UI is usable, then
    // fetch the real bootstrap data from the backend API.
    replaceState({});
    getJson("/api/bootstrap/")
      .then(function (payload) {
        replaceState(payload);
        // Re-apply routing after state is loaded
        syncRouteGlobals();
      })
      .catch(function (err) {
        console.warn("[ela] Could not load bootstrap from API:", err.message);
      });
  }
